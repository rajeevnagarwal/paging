...
select_a_victim(pde_t *pgdir)
{
	...
	while(victim==0)
	{
		for(i=0;(i < NPDENTRIES) && (victim == 0) && ((PGSIZE * NPTENTRIES * i) < KERNBASE);i++)
		{
			if(pgdir[i]&PTE_P)
			{
				pde = (pte_t*)P2V(PTE_ADDR(pgdir[i]));				
				for(j=0;j < NPTENTRIES && ((PGSIZE * NPTENTRIES * i + j * PGSIZE) < KERNBASE);j++)
				{
					if(pde[j]&PTE_P)
					{
						if(!(pde[j]&PTE_A))
...
clearaccessbit(pde_t *pgdir)
{
	uint i, j;
	 pte_t *pte;
  int cutoff = (MY_RAND_MAX) / 10;
  //#ifdef PRINT_DEBUG_STATEMENTS
  int n = 0;
  int reset = 0;
  //#endif

  for(i = 0; i < NPDENTRIES && ((PGSIZE * NPTENTRIES * i) < KERNBASE); i++){
    if(pgdir[i] & PTE_P){
